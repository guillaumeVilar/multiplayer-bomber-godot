...
gcp-deploy-cloud-functions:
	gcloud functions deploy stopAppEngineService --runtime python311 --entry-point=stop_appengine_service --source ./builds/gcp/stop_start_app_engine_functions --trigger-http --allow-unauthenticated --max-instances=1 --memory=128Mi --gen2 --region europe-west1
	gcloud functions deploy startAppEngineService --runtime python311 --entry-point=start_appengine_service --source ./builds/gcp/stop_start_app_engine_functions --trigger-http --allow-unauthenticated --max-instances=1 --memory=128Mi --gen2 --region europe-west1